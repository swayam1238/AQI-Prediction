...
TWILIO_ACCOUNT_SID = 'AC2ed8c97da8206219b11dc33e38a86b22'  # Replace with your actual Twilio SID
TWILIO_AUTH_TOKEN = '9845dd1547f00611c421152c2e18978a'  # Replace with your actual Twilio Auth Token
TWILIO_PHONE_NUMBER = '+17754167203'  # Replace with your Twilio phone number
USER_PHONE_NUMBER = '+919448265129'  # Replace with the user's phone number
...
def get_predictions_for_future_hours(input_data, model, scaler, hours_list):
    ...
    for hours in hours_list:
        pred = []
        temp_input = input_data.copy()  # Create a copy of the input data to modify
        for i in range(hours):
            # Make a prediction for this time step
            prediction = model.predict(temp_input)
            pred.append(prediction[0])  # Append the prediction for the current time step
            # Update the input for the next prediction (if necessary)
            if i < hours - 1:  # If there are still more steps, shift the input data
                temp_input[0, i+1, 0:10] = prediction[0, :10]  # Update the features
                temp_input[0, i+1, 1:] = temp_input[0, i, 1:]  # Keep the pollutants constant (or modify based on your data)
...
if tabs == "Real-Time AQI":
    ...
    if aqi_data:
        pollutants = ["aqi"]  # Only AQI will be included, no temperature, wind speed, or weather condition
        real_time_values = [aqi_data['aqi']]
        for pollutant, value in aqi_data['iaqi'].items():
            if pollutant not in ['t', 'wind', 'wg', 'w']:  # Exclude temperature (t), wind (wind), weather (wg), and wind direction (w)
                pollutants.append(pollutant)
                real_time_values.append(value['v'])

        # Ensure the data has at least 10 values for all pollutants
        while len(real_time_values) < 10:
            real_time_values.append(0)

        # Prepare input data for prediction (same as before)
        ...
        hours_list = [1, 6, 18]  # Now showing for 1st hour, 6th hour, and 18th hour
        ...
        # Display the predictions in one table
        st.markdown(f"### Air Quality Predictions for the Next 1, 6, and 18 Hours")
        prediction_table = {}
        for i in range(3):  # Display predictions for 1st, 6th, and 18th hours
            hour = [1, 6, 18][i]
        ...
        # Send SMS notification for predictions (if needed)
        send_sms_notification(future_predictions[18], pollutants, 18)
    else:
